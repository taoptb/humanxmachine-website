...
function CourseCard({ course }: { course: Course }) {
  return (
    <div className="bg-white border border-[#E2E2DF] rounded overflow-hidden group">
      <div className="relative h-48 overflow-hidden">
        {course.thumbnailUrl ? (
          ...
          <div className="w-full h-full bg-[#F0F0ED] flex items-center justify-center">
            <span className="font-mono text-[9px] tracking-[2px] text-[#bbb] uppercase">Course</span>
          </div>
        )}
      </div>
      <div className="p-6">
        <p className="font-mono text-[9px] tracking-[2px] text-orange uppercase mb-2">{course.category}</p>
        <h3 className="font-headline font-bold text-[#12120f] text-lg tracking-tight mb-3">{course.title}</h3>
        <p className="text-[13px] text-[#888] leading-relaxed mb-5">{course.description}</p>
        <div className="flex items-center justify-between">
          <span className="font-headline font-bold text-[#12120f] text-lg">
  ...
  return (
    <main className="bg-[#F7F7F7] min-h-screen pt-24 px-6 md:px-10 pb-20">
      <div style={{ maxWidth: '1440px', margin: '0 auto' }}>
        ...
        <p className="text-[14px] text-[#888] mb-12 max-w-xl">
        ...
        {courses.length === 0 ? (
          <div className="text-center py-32">
            ...
            <p className="text-[#aaa] mt-3 text-sm">Our first courses are in production.</p>
          </div>
        ) : (
